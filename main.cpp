...
int main() {
    ...
    ifstream input;
    ofstream foutput("/tmp/out.raw", std::ifstream::out | std::ifstream::binary);
    ostringstream output;
    size_t outBytes = 0;

    input.open("/tmp/sample.mp3", std::ifstream::in | std::ifstream::binary);
    while (input.good()) {
        input.read(reinterpret_cast<char *>(inbuf), INBUFF);
        streamsize len = input.gcount();
        size_t size;
        mpg123_format_none(m);
        mpg123_format(m, 48000, 2, MPG123_ENC_FLOAT_32);
